...
def get_mapit_type_for_pombola_placekind(placekind):
    ...
    if len(types_in_mapit) == 0:
        raise_(Exception, "Couldn't find a MapIt type that corresponded to the PlaceKind %s" % (placekind,))
    elif len(types_in_mapit) == 1:
        return list(types_in_mapit)[0]
    else:
        raise_(Exception, "There were multiple MapIt types (%s) corresponding to the PlaceKind %s" % (", ".join(types_in_mapit), placekind))
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for arg in args:
            m = re.search('([-\w]+):([-\w]+)', arg)
            if not m:
                raise_(CommandError, "The argument '%s' was malformed" % (arg,))
