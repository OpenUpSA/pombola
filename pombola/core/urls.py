...
person_patterns = [
    url(r"^all/", ListView.as_view(model=models.Person), name="person_list"),
    url(
        r"^politicians/",
...
place_patterns = [
    ...
    url(
        r"^is/(?P<slug>[-\w]+)/(?P<session_slug>[-\w]+)/?",
        PlaceKindList.as_view(),
        name="place_kind",
    ),
...
urlpatterns = [
    ...
    url(
        r"^position/(?P<pt_slug>[-\w]+)/(?P<ok_slug>[-\w]+)/$",
        position_pt_ok,
        name="position_pt_ok",
    ),
    url(
        r"^position/(?P<pt_slug>[-\w]+)/(?P<ok_slug>[-\w]+)/(?P<o_slug>[-\w]+)/$",
        position_pt_ok_o,
        name="position_pt_ok_o",
    ),
...
urlpatterns += (url(r"^status/memcached/", memcached_status, name="memcached_status"),)

if settings.POPIT_API_URL:
    # Add the dynamically generated API help page
    urlpatterns.append(url(r"^help/api", HelpApiView.as_view(), name="help-api"))
...
urlpatterns += (url(r"robots.txt", robots),)

# Serve favicon.ico at root too
urlpatterns += (
    url(r"^favicon\.ico$", RedirectView.as_view(url="/static/images/favicon.ico")),
)

# A URL for testing jQuery UI components:
urlpatterns += (url(r"^ui-test", TemplateView.as_view(template_name="ui-test.html")),)

urlpatterns += (
    url(r"version.json", VersionView.as_view(), name="version"),
    url(
        r"template-for-blog.html",
        TemplateView.as_view(template_name="template-for-blog.html"),
    ),
    url(
        r"elasticsearch-health",
        ElasticSearchHealthView.as_view(),
