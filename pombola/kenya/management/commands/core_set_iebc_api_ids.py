...
def get_matching_party(party_name, **options):
    ...
    if len(matching_parties) == 0:
        if options.get('create', True):
            ...
            raise_(CommandError, "Would have to create %s, but 'create' was False" % (party_name,))
    ...
    def handle_noargs(self, **options):
        ...
        for party in party_data['parties']:
            ...
            try:
                ...
                print("Not setting the API code: %s" % (ce,), file=sys.stderr)
            ...
            for i, area in enumerate(areas):
                ...
                for race in candidate_data['candidates']:
                    ...
                    for candidate in candidates:
                        ...
                        if not person:
                            raise_(Exception, "Failed to find the person from '%s' '%s'" % (first_names, surname))
                        ...
                        if positions:
                            if len(positions) > 1:
                                print("There were multiple (%d) matches for: %s" % (len(positions), aspirant_position_properties,), file=sys.stderr)
